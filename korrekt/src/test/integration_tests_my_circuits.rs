...
mod tests {
    ...
    use crate::io::{
        analyzer_io_type,
        analyzer_io_type::{AnalyzerOutputStatus, LookupMethod, VerificationInput, VerificationMethod},
    };
    ...
    fn test_underconstrained_decoder_detected() {
        ...
        let mut analyzer = Analyzer::new(
            &circuit, k, AnalyzerType::UnderconstrainedCircuit, Some(&analyzer_input),
        ).unwrap();
        let output_status = analyzer.analyze_underconstrained(&analyzer_input).unwrap().output_status;
